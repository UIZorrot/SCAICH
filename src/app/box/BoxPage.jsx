        ...
        <div className="hero-section1">
          ...
          >
            ...
            <Paragraph className="hero-subtitle">
              Your personal academic workspace for managing research and publications.
            </Paragraph>
          </motion.div>
        </div>
...
const FavoritesTab = ({ favorites, onRemove, currentTheme, onDeepResearch }) => {
    ...
    try {
      ...
      if (savedCategories) {
        ...
        const defaultCategories = [
          { id: 'uncategorized', name: '未分类', color: 'gray', isDefault: true }
        ];
  ...
  const filteredFavorites = favorites.filter(paper => {
    const matchesKeyword = !searchKeyword ||
      ...
      paper.author.toLowerCase().includes(searchKeyword.toLowerCase()) ||
      (paper.abstract && paper.abstract.toLowerCase().includes(searchKeyword.toLowerCase()));

    const matchesCategory = selectedCategory === 'all' ||
      paper.category === selectedCategory ||
      (!paper.category && selectedCategory === 'uncategorized');

    const matchesTags = selectedTags.length === 0 ||
      selectedTags.every(tag => paper.tags && paper.tags.includes(tag));
  ...
  const categoryOptions = [
    { id: 'all', name: '全部论文', color: 'blue', icon: <FolderOutlined /> },
    ...categories
  ];
  // 查看论文处理函数
  ...
  const hasFulltext = (paper) => {
    ...
    return (
      ...
      (paper.url && paper.url.toLowerCase().includes('arxiv'))
    );
  };
  ...
  const handleDeepResearch = (paper) => {
    ...
    if (paper.doi) {
      ...
      if (paper.doi.toLowerCase().includes('arxiv') ||
        (paper.url && paper.url.toLowerCase().includes('arxiv'))) {
        source = 'arxiv';
      }
  ...
  const updatePaperInFavorites = (updatedPaper) => {
    try {
      ...
      const updatedFavorites = favorites.map(fav =>
        fav.doi === updatedPaper.doi ? updatedPaper : fav
      );
      localStorage.setItem('scai_favorites', JSON.stringify(updatedFavorites));
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                <div style={{ flex: 1, marginRight: '1rem' }}>
                  ...
                  <div style={{ marginBottom: '0.75rem', display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'center' }}>
                    {paper.category && (
                      <Tag
                        color={categories.find(cat => cat.id === paper.category)?.color || 'blue'}
                        style={{ margin: 0 }}
                      >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', minWidth: '140px' }}>
                  ...
                  <Button
                    ...
                    className="modern-btn modern-btn-secondary"
                    size="small"
                    disabled={!hasFulltext(paper)}
                    onClick={() => handleDeepResearch(paper)}
                    title={!hasFulltext(paper) ? "该论文暂无全文可用" : "深度研究论文"}
                  >
                    Deep Research
                  </Button>
                  <Button
                    ...
                    className="modern-btn modern-btn-secondary"
                    size="small"
                    onClick={() => handleBibTexClick(paper)}
                  >
                    BibTeX
                  </Button>
                  <Button
        ...
        {currentPaper && (
          <div>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <Text strong>分类:</Text>
              <Select
                value={currentPaper.category || 'uncategorized'}
                onChange={(value) => handleCategoryChange(currentPaper, value)}
            ...
            <div style={{ display: 'flex', gap: '1rem', alignItems: 'end' }}>
              <div style={{ flex: 1 }}>
                ...
                <Input
                  ...
                  placeholder="输入分类名称"
                  onPressEnter={editingCategory ? handleUpdateCategory : handleCreateCategory}
                />
              </div>
              <div>
                <Text>颜色:</Text>
              ...
              {categories.map(category => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    {category.isDefault && (
                      <Tag size="small" color="orange">默认</Tag>
                    )}
                  </div>
...
const UploadsTab = ({ uploads, onUpload, currentTheme, onRemoveUpload }) => {
  ...
  const handleRemoveUpload = (upload) => {
    ...
    const confirmText = upload.uploadMode === 'irys'
      ? '确定要隐藏这个文档吗？文档仍会保存在Irys网络上，但不会在此处显示。'
      : '确定要删除这个文档吗？此操作不可撤销。';
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      ...
                      <Button
                        icon={upload.uploadMode === 'irys' ? <EyeInvisibleOutlined /> : <DeleteOutlined />}
                        onClick={() => handleRemoveUpload(upload)}
                        className="modern-btn modern-btn-secondary"
                        size="small"
                        danger={upload.uploadMode !== 'irys'}
                      >
    ...
    <div className="modern-card" style={{ padding: '2rem' }}>
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <Avatar
          ...
          icon={!(profileData?.avatarUrl || user?.imageUrl) && <UserOutlined />}
        ...
        {profileData?.position && (
          <Text style={{ color: currentTheme.isDark ? '#ccc' : '#666', display: 'block', marginBottom: '0.5rem' }}>
            {profileData.position}
          </Text>
        )}
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          {profileData?.website && (
            <div style={{ marginBottom: '0.5rem' }}>
              <Text style={{ color: currentTheme.isDark ? '#ccc' : '#666' }}>
                其他个人网站: <a href={profileData.website} target="_blank" rel="noopener noreferrer">{profileData.website}</a>
              </Text>
            </div>
          )}
          {profileData?.orcid && (
            <div>
              <Text style={{ color: currentTheme.isDark ? '#ccc' : '#666' }}>
                ORCID: <a href={`https://orcid.org/${profileData.orcid}`} target="_blank" rel="noopener noreferrer">{profileData.orcid}</a>
              </Text>
            </div>
          )}
        </div>
...
const UploadModal = ({ visible, onClose, onSuccess, user }) => {
  ...
  const handleUpload = async (values) => {
    ...
    try {
      ...
      const uploadResult = await uploadToIrys(actualFile, {
        title: values.title,
  ...
  return (
    <Modal
      title="Upload Document to SCAI Box"
      open={visible}
      onCancel={onClose}
      footer={null}
      width={900}
    ...
    >
      ...
      >
        ...
        <Form.Item
          ...
          extra="Private documents will be encrypted and only accessible to you"
        >
          <Switch
        ...
        {fileType === 'literature' && (
          <>
            <Form.Item
              name="doi"
              label="DOI"
              rules={[{ required: true, message: '文献类型文件必须输入DOI' }]}
              extra="输入DOI后将自动获取论文信息"
            >
              <Input
                placeholder="例如: 10.1038/nature12373"
                onChange={(e) => handleDoiSearch(e.target.value)}
                suffix={doiLoading && <LoadingOutlined />}
              />
            </Form.Item>
                ...
                <div style={{
                  ...
                  {paperMetadata.referencecount > 0 && (
                    <div>
                      ...
                      <div style={{ fontSize: '14px', color: '#333' }}>
                        {paperMetadata.referencecount.toLocaleString()} 次
                      </div>
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '14px', color: paperMetadata.is_oa ? '#52c41a' : '#ff4d4f' }}>
                      {paperMetadata.is_oa ? '✅ 是' : '❌ 否'}
                    </div>
                  </div>
                ...
                {paperMetadata.abstract && paperMetadata.abstract !== "Abstract Not Available" && (
                  <div>
                    ...
                    <div style={{
                      ...
                      {paperMetadata.abstract.length > 500
                        ? `${paperMetadata.abstract.substring(0, 500)}...`
        ...
        <Form.Item label="Select File">
          <Upload.Dragger {...uploadProps}>
            ...
            <p className="ant-upload-hint">
              Support for PDF, text, and document files. Maximum file size: 10MB
            </p>
          </Upload.Dragger>
        </Form.Item>
        ...
        {uploading && (
          <Form.Item>
            ...
            <div style={{ textAlign: 'center', marginTop: '8px', color: '#666' }}>
              {uploadMode === 'irys' ? 'Uploading to Irys network...' : 'Uploading to local storage...'}
            </div>
          </Form.Item>
        )}
            ...
            {contributions.map((contribution, index) => (
              <div key={index} style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
                <Input.TextArea
                  ...
                  placeholder={`贡献 ${index + 1}: 例如：发表高影响因子论文、获得重要奖项、主持重大项目等`}
                  rows={2}
                  style={{ flex: 1 }}
                />
